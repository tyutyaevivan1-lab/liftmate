...
Фронтенд должен присылать initData в заголовке `X-Telegram-Init-Data` при каждом
запросе (Telegram.WebApp.initData — уже готовая строка, ничего парсить самим не
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[WEBAPP_URL, "http://localhost:8765", "http://127.0.0.1:8765"],
    allow_methods=["GET"],
    allow_headers=["*"],
)
...
async def require_telegram_user(
    ...
    """FastAPI-зависимость: проверяет initData и возвращает провалидированного пользователя Telegram."""
    try:
        return _validate_init_data(x_telegram_init_data, BOT_TOKEN)
